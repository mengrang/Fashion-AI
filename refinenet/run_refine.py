...
'''
    load model
'''
refine_model = importlib.import_module('models.nets.' + FLAGS.network_def)
...
def train():
    ...
    model_save_dir = os.path.join('logs_and_weights_refine3',
                                  'models',
                                  'weights',
                                  model_path_suffix)
    train_log_save_dir = os.path.join('logs_and_weights_refine3',
                                      'models',
                                      'logs',
                                      model_path_suffix,
                                      'train')
    test_log_save_dir = os.path.join('logs_and_weights_refine3',
                                     'models',
                                     'logs',
                                     model_path_suffix,
                                     'test')
    ...
    """
    refinenet = refine_model.RefineNet(total_num = FLAGS.total_num,
    ...
    refinenet.build_loss3(optimizer='Adam')
    print('=====RefineNet Model Build=====\n')
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count, allow_soft_placement=True)) as sess:
        ...
        '''
        # Restore RefineNet pretrained weights
        if FLAGS.pretrained_model != '':
            if FLAGS.pretrained_model.endswith('.pkl'):
                refinenet.load_weights_from_file(FLAGS.pretrained_model, sess, finetune=True)
                # Check weights
        ...
        for training_itr in range(FLAGS.training_iters):
            ...
            batch_x_np, batch_x_resized, batch_predicted_heatmap_np, batch_gt_heatmap_np, batch_centermap, batch_weight_np = next(generator)
            ...
            refine_loss_np, _, summaries, current_lr, \
            refined_heatmap_np, global_step = sess.run([refinenet.refine_loss,
                                                        refinenet.train_op,
                                                        merged_summary,
                                                        refinenet.lr,
                                                        refinenet.refined_heatmap,
                                                        refinenet.global_step
                                                        ],
                                                        feed_dict={refinenet.input_images: batch_x_np,
                                                                   refinenet.cmap_placeholder: batch_centermap,
                                                                   refinenet.gt_hmap_placeholder: batch_gt_heatmap_np,
                                                                   refinenet.train_weights_placeholder: batch_weight_np,
                                                                   refinenet.input_images_resized_placeholder: batch_x_resized,
                                                                   refinenet.predicted_heatmap_placeholder: batch_predicted_heatmap_np})

            # Show training info
            print_current_training_stats(global_step, current_lr, refine_loss_np, time.time() - t1)
            ...
            if FLAGS.if_show:
                ...
                if (global_step + 1) % FLAGS.img_show_iters == 0:
                    ...
                    input_heatmap = batch_predicted_heatmap_np[0, :, :, 0:FLAGS.num_of_joints].reshape(
                        (FLAGS.input_size, FLAGS.input_size, FLAGS.num_of_joints))
                    input_heatmap = np.amax(input_heatmap, axis=2)
                    input_heatmap = np.reshape(input_heatmap, (FLAGS.input_size, FLAGS.input_size, 1))
                    input_heatmap = np.repeat(input_heatmap, 3, axis=2)

                    demo_stage_heatmap = refined_heatmap_np[0, :, :, 0:FLAGS.num_of_joints].reshape(
                    ...
                    upper_img = np.concatenate((input_heatmap, demo_stage_heatmap), axis=1)
                    lower_img = np.concatenate((demo_gt_heatmap, blend_img), axis=1)
            ...
            if (global_step + 1) % FLAGS.validation_iters == 0:
                ...
                while cnt < 10:
                    batch_x_np, batch_x_resized, batch_predicted_heatmap_np, batch_gt_heatmap_np, batch_centermap, batch_weight_np = next(generator_eval)
                    ...
                    #                                                          FLAGS.joint_gaussian_variance,
                    #                                                          batch_joints_np)
                    refine_loss_np, summaries = sess.run([refinenet.refine_loss, merged_summary],
                                                         feed_dict={refinenet.input_images: batch_x_np,
                                                                   refinenet.cmap_placeholder: batch_centermap,
                                                                   refinenet.gt_hmap_placeholder: batch_gt_heatmap_np,
                                                                   refinenet.train_weights_placeholder: batch_weight_np,
...
def print_current_training_stats(global_step, cur_lr, refine_loss_np, time_elapsed):
    stats = 'Step: {}/{} ----- Cur_lr: {:1.7f} ----- Time: {:>2.2f} sec.'.format(global_step, FLAGS.training_iters,
                                                                                 cur_lr, time_elapsed)
    losses = 'refine loss: {:>7.2f}\n'.format(refine_loss_np)
    print(stats)
    print(losses + '\n')
