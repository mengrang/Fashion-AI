...
class DataGenerator():
    ...
    def __init__(self, total_joints_list, blouse_joints_list, dress_joints_list, outwear_joints_list, skirt_joints_list,
                 ...
                 img_dir, train_data_file, pred_data_file):
        """Initializer
            Args:
    ...
    def _read_pred_data(self):
        """
        To read labels in csv
        """
        self.pred_data_dict = {}       # The labels of images
        label_file = pd.read_csv( self.pred_data_file)
        print('READING LABELS OF PRED DATA')
        print('Total num:', label_file.shape[0])
        ...
        for i in range(label_file.shape[0]):
            ...
            joints = np.reshape(joints, (-1, 2))
            self.pred_data_dict[name] = {'joints': joints}
        print('PRED LABEL READING FINISHED\n')
        return self.pred_data_dict
    ...
    def _rotate_augment(self, img, hm, pred_hm, max_rotation=45):
        ...
        if random.choice([0, 1]):
            ...
            pred_hm = transform.rotate(pred_hm, r_angle)
        return img, hm, pred_hm

    # input image size=512,out image size=512,input joints size = 512
    def _size_augment(self, img, joints, pred_joints, train_weights, min_compress_ratio=0.7, max_compress_ratio=1.35):
        aug_joints = np.copy(joints)
        pred_aug_joints = np.copy(pred_joints)
        ...
        if compress_ratio <= 1.0:
            ...
            # calculate relative joints after size augmentation
            pred_compress_coord = pred_joints * compress_ratio  # refer to compressed img joints coordinate
            pred_hm_x_padding = img_x_padding
            pred_hm_y_padding = img_y_padding
            pred_aug_joints[:, 1] = pred_compress_coord[:, 1] + pred_hm_x_padding  # padding joints to 512
            pred_aug_joints[:, 0] = pred_compress_coord[:, 0] + pred_hm_y_padding
            pred_aug_joints = pred_aug_joints * 64.0 / 512.0  # resize to 64
            pred_aug_joints = pred_aug_joints.astype(np.int32)
            ...
            # print('compress ratio', compress_ratio)
        return aug_img, aug_joints, pred_aug_joints, train_weights
    ...
    def _aux_generator(self, batch_size=16, normalize=False, sample_set='train'):
        ...
        while True:
            ...
            train_img_resized = np.zeros((batch_size, 64, 64, 3), dtype=np.float32)
            train_gtmap = np.zeros((batch_size, 64, 64, len(self.total_joints_list)), np.float32)
            pred_heatmap = np.zeros((batch_size, 512, 512, len(self.total_joints_list)), np.float32)
            ...
            while i < batch_size:
                ...
                pred_joints = self.pred_data_dict[name]['joints']
                ...
                # process img to 512
                _, pred_joints = self._process_train_img_to_center512(img, pred_joints)
                ...
                if random.choice([0, 1]):
                    ...
                    new_pred_joints = self._relative_joints(cbox, padd, pred_joints, to_size=512)
                    img, new_j, new_pred_joints, train_weights[i] = self._size_augment(img, new_j, new_pred_joints, train_weights[i])
                    hm = self._generate_hm(64, 64, dress_type, new_j, 64, train_weights[i])
                    pred_hm = self._generate_hm(64, 64, dress_type, new_pred_joints, 64, train_weights[i])
                else:
                    new_joints = self._relative_joints(cbox, padd, joints, to_size=64)
                    new_pred_joints = self._relative_joints(cbox, padd, pred_joints, to_size=64)
                    hm = self._generate_hm(64, 64, dress_type, new_joints, 64, train_weights[i])
                    pred_hm = self._generate_hm(64, 64, dress_type, new_pred_joints, 64, train_weights[i])

                # rotate augmentation
                img, hm, pred_hm = self._rotate_augment(img, hm, pred_hm)

                train_gtmap[i] = hm
                pred_heatmap[i] = cv2.resize(pred_hm, (512, 512))
                ...
                if normalize:
                    ...
                    train_img_resized[i] = cv2.resize(train_img[i], (64, 64))
                else:
            ...
            yield train_img, train_img_resized, pred_heatmap, train_gtmap, train_centermap, loss_weights
    ...
    def test2(self):
        ...
        while True:
            ...
            if random.choice([1]):
                new_j = self._relative_joints(cbox, padd, joints, to_size=512)
                new_pred_joints = self._relative_joints(cbox, padd, pred_joints, to_size=512)
                img, new_j, new_pred_joints, train_weights = self._size_augment(img, new_j, new_pred_joints,
                                                                                   train_weights)
                hm = self._generate_hm(64, 64, dress_type, new_j, 64, train_weights)
                pred_hm = self._generate_hm(64, 64, dress_type, new_pred_joints, 64, train_weights)
            else:
                new_joints = self._relative_joints(cbox, padd, joints, to_size=64)
                new_pred_joints = self._relative_joints(cbox, padd, pred_joints, to_size=64)
                hm = self._generate_hm(64, 64, dress_type, new_joints, 64, train_weights)
                pred_hm = self._generate_hm(64, 64, dress_type, new_pred_joints, 64, train_weights)
            ...
            train_gtmap = hm
            pred_heatmap = cv2.resize(pred_hm, (512, 512))

            hm2 = np.amax(train_gtmap, axis=2)
            hm2 = cv2.resize(hm2, (512, 512))
            cv2.imshow('hm', hm2)
            cv2.waitKey(1000)

            hm2 = np.amax(pred_heatmap, axis=2)
            # hm2 = cv2.resize(hm2, (512, 512))
            print(hm2.shape)
            cv2.imshow('hm', hm2)
            cv2.waitKey(1000)
