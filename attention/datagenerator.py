...
class DataGenerator():
        ...
        while True:
            ...
            while i < batch_size:
                ...
                if random.choice([0, 1]) == 1:
                    ...
                    for i in range(1,24):
                        dm[i-1] = cv2.imread(os.path.join(self.dm_dir, 'heat' ,str(i) ,'.png'),0)
                        dm[i-1] = self._dm_size_augment(dm, compress_ratio)
    ...
    def open_dm(self, dm_dir, color='GRAY'):
        """ Open an dm
        Args:
            name	: Name of the sample
            color	: Color Mode (RGB/BGR/GRAY)
        ...
        for root, dirs, files in os.walk(dm_dir):
            for file in files:
                dm = cv2.imread(os.path.join(self.dm_dir, file))
                # if color == 'RGB':
