...
sys.path.append('/home/lzhpc/home/gu/add occlusion and fpn/version1_3/models/nets')
...
class CPM_Model(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('sub_stages'):
            ...
            sub_unit_1 = bottleneck(inputs=self.input_images,
                            depth=64,
                            depth_bottleneck=32,
                            stride=1,
                            name='sub_unit_1'
                            )
            sub_unit_2 = bottleneck(inputs=sub_unit_1,
                            depth=64,
                            depth_bottleneck=32,
                            stride=1,
                            name='sub_unit_2'
            ...
            sub_unit_3 = tf.layers.conv2d(inputs=sub_pool1,
                                ...
                                name='sub_unit_3')      
            sub_unit_4 = bottleneck(inputs=sub_unit_3,
                            depth=128,
                            depth_bottleneck=32,
                            stride=1,
                            name='sub_unit_4'
                            )
            sub_pool2 = tf.layers.average_pooling2d(inputs= sub_unit_4,
                        pool_size=[2, 2],
                        strides=[2,2],
                        padding='same',     
                        name='sub_pool2')
            ...
            sub_unit_5 = tf.layers.conv2d(inputs=sub_pool2,
                                ...
                                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                name='sub_unit_5')         
            sub_unit_6 = bottleneck(inputs=sub_unit_5,
                                depth=256,
                                depth_bottleneck=64,
                                stride=1,
                                name='sub_unit_6'
                                )   
            sub_unit_7 = bottleneck(inputs=sub_unit_6,
                            depth=256,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_7'
                            )
            sub_unit_8 = bottleneck(inputs=sub_unit_7,
                            depth=256,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_8'
                            )
            sub_pool3 = tf.layers.average_pooling2d(inputs=sub_unit_8,
                        pool_size=[2, 2],
                        strides=[2,2],
                        padding='same',     
                        name='sub_pool3')
            ...
            sub_unit_9 = tf.layers.conv2d(inputs=sub_pool3,
                                ...
                                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                name='sub_unit_9')   
            sub_unit_10 = bottleneck(inputs=sub_unit_9,
                            depth=512,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_10')
            sub_unit_11= tf.layers.conv2d(inputs=sub_unit_10,
                                ...
                                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                name='sub_unit_11') 
            sub_unit_12 = bottleneck(inputs=sub_unit_11,
                            depth=256,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_12')
            sub_unit_13 = bottleneck(inputs=sub_unit_12,
                            depth=256,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_13')
            sub_unit_14 = bottleneck(inputs=sub_unit_13,
                            depth=256,
                            depth_bottleneck=64,
                            stride=1,
                            name='sub_unit_14')
            ...
            self.sub_fms = [sub_pool4,sub_pool3, sub_pool2,sub_pool1]     #[32,64,128,256]
            # FPN block
            for i, fm in enumerate(self.sub_fms):     #blocks is the param of the creat_global_net
                # with tf.variable_scope('lateral'):
                '''lateral 1*1conv'''
                lateral =tf.layers.conv2d(inputs=fm,
                                filters=64,
                                kernel_size=1,
                                strides=(1,1),
                                activation=tf.nn.relu,
                                name='lateral{}_conv'.format(5-i))   # channel = 128
                last_fm = lateral
                self.lateral_fms.append(last_fm)
            p5 = self.lateral_fms[0]
            p5_2 = self.conv2d_transpose_strided(x=p5,
                                W=tf.get_variable(name="W_p5", shape=[3, 3, 64,64]),
                                b=tf.get_variable(name="b_p5", shape=[64]),
                                output_shape=tf.shape(self.lateral_fms[1]))   #size=64
            
            p4 = tf.add(p5_2, self.lateral_fms[1], name="p4")             #size=64
            p4_2 = tf.image.resize_bilinear(p4,(128,128),name='p2')         #size=128   
            p3 = tf.add(p4_2, self.lateral_fms[2], name="p3")             #size=128
            p2 = tf.add(tf.image.resize_bilinear(p3,(256, 256)), self.lateral_fms[3])   #size=256
            p2_half = tf.image.resize_bilinear(p2,(128,128),name='p2_half')       #size=128
            p_cat = tf.concat([p2_half, p3, p4_2, tf.image.resize_bilinear(p5_2,(128,128))],axis=3)
        ...
        with tf.variable_scope('', reuse=True):
            ...
            for i in range(1, 3):
                for j in range(1,4):
                    ...
                    conv_bias = tf.get_variable('sub_stages' + '/unit_' +str(i+10)  + '/conv' +str(j)+ '/bias')
