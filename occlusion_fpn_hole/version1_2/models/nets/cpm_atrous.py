...
class CPM_Model(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('sub_stages'):
            ...
            sub_conv4_1 = tf.layers.conv2d(inputs=sub_conv3,
                                         filters=128,
                                         kernel_size=[3, 3],
                                         strides=[1, 1],
                                         padding='same',
            ...
            sub_conv4_2 = tf.layers.conv2d(inputs=sub_conv3,
                                         filters=128,
                                         kernel_size=[3, 3],
                                         strides=[1, 1],
                                         padding='same',
                                         ...
                                         kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                         name='sub_conv4_2')
            sub_conv4_cat = tf.concat([sub_conv4_1,sub_conv4_2],axis=3)
            # channel = 256, size = 128
            sub_pool2 = tf.layers.max_pooling2d(inputs=sub_conv4_cat,
                                                pool_size=[2, 2],
                                                strides=2,
                                                padding='same',  
                                                name='sub_pool2')
    ...
    def build_loss3(self, optimizer='Adam'):
        ...
        with tf.variable_scope('train'):
            ...
            new_lr = tf.cond(tf.less(epcho, tf.constant(4.0, dtype=tf.float64)),
                             lambda: 0.0002 / 10.0 * epcho,
                             lambda: tf.cond(tf.less(epcho, tf.constant(5.0, dtype=tf.float64)),
                                     lambda: tf.constant(0.0005, dtype=tf.float64),
                                     lambda: tf.cond(tf.less(epcho, tf.constant(10.0, dtype=tf.float64)),
                                             lambda: 0.0005 -((epcho - 5.0) / (10.0 - 5.0)*0.00048),
    ...
    def load_weights_from_file(self, weight_file_path, sess, finetune=True):
        ...
        with tf.variable_scope('', reuse=True):
            ...
            for layer in range(1, 3):
                ...
                loaded_kernel = weights['sub_stages'+'/sub_conv' + str(layer)]
            ...
            for layer in range(1, 2):
                ...
                loaded_kernel = weights['sub_stages/sub_conv' + str(layer + 2) + str(layer)]
                loaded_bias = weights['sub_stages/sub_conv' + str(layer + 2) + str(layer) + '_b']

                sess.run(tf.assign(conv_kernel, loaded_kernel))
                sess.run(tf.assign(conv_bias, loaded_bias))
            #conv4_x
            for layer in range(1, 3):
                conv_kernel = tf.get_variable('sub_stages/sub_conv4_' + str(layer) + '/kernel')
                conv_bias = tf.get_variable('sub_stages/sub_conv4_' + str(layer) + '/bias')

                loaded_kernel = weights['sub_stages/sub_conv4_' + str(layer)]
                loaded_bias = weights['sub_stages/sub_conv4_' + str(layer) + '_b']
            ...
            for layer in range(1, 4):
                ...
                loaded_kernel = weights['sub_stages/sub_conv' + str(layer + 4)]
                loaded_bias = weights['sub_stages/sub_conv' + str(layer + 4) + '_b']
            ...
            for layer in range(1, 4):
                ...
                loaded_kernel = weights['sub_stages/sub_conv' + str(layer + 10) ]
                loaded_bias = weights['sub_stages/sub_conv' + str(layer + 10)+'_CPM_b']
