...
class CPM_Model(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('sub_stages'):
            ...
            # FPN block
            C5 = sub_pool4  # channel = 256, size = 64
            C4 = sub_pool3  # channel = 256, size = 128
            ...
            C3_conv1 = tf.layers.conv2d(inputs=C3,
                                        ...
                                        padding='same',
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                        name='C3_conv1')    # channel = 256, size = 128
            '''
            ...
            # P5 2x
            W_P5 = tf.get_variable(name="W_P5", shape=[3, 3, 256, 256])
            b_P5 = tf.get_variable(name="b_P5", shape=[256])
            P5_2x = self.conv2d_transpose_strided(P5, W_P5, b_P5, tf.shape(C4_conv1))     # channel = 256, size = 128
            ...
            P4 = tf.add(C4_conv1, P5_2x, name="P4")      # channel = 256, size = 128
            # # P4_2x
            ...
            # P3
            P3 = tf.add(C3_conv1, P4, name="P3")

            P_cat = tf.concat([P3, P4, P5_2x], axis=3)       # channel = 256, size = 128
                                     ...
                                     lambda: tf.cond(tf.less(epcho, tf.constant(10.0, dtype=tf.float64)),
                                             ...
                                             lambda: tf.cond(tf.less(epcho, tf.constant(20.0, dtype=tf.float64)),
                                                 ...
                                                 lambda: tf.cond(tf.less(epcho, tf.constant(30.0, dtype=tf.float64)),
                                                         ...
                                                         lambda: 0.00001 - ((epcho - 30.0) / (10.0) * 0.000005)
                             )))))
            self.lr = new_lr
            tf.summary.scalar('learning rate', self.lr)
