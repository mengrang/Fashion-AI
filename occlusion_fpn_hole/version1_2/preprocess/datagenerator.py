...
class DataGenerator():
    ...
    def _size_augment(self, img, joints, train_weights, min_compress_ratio=0.7, max_compress_ratio=1.35):
        ...
        if compress_ratio <= 1.0:
            ...
            aug_joints = aug_joints * 128.0 / 512.0  # resize to 128
            ...
            for elem in range(len(train_weights)):
                if train_weights[elem] != -1:
                    if aug_joints[elem][0] < 0 or aug_joints[elem][1] < 0 or \
                       aug_joints[elem][0] > 128 or aug_joints[elem][1] > 128:
    ...
    def _aux_generator(self, batch_size=16, normalize=False, sample_set='train'):
        ...
        while True:
            ...
            train_gtmap = np.zeros((batch_size, 128, 128, len(self.total_joints_list)), np.float32)
            ...
            while i < batch_size:
                ...
                if random.choice([0, 1]) == 1:
                    ...
                    hm = self._generate_hm(128, 128, dress_type, new_j, 128, train_weights[i])
                else:
                    new_j = self._relative_joints(cbox, padd, joints, to_size=128)
                    hm = self._generate_hm(128, 128, dress_type, new_j, 128, train_weights[i])
    ...
    def generator(self, batchSize=8, norm=False, sample='train'):
    ...
    def test2(self):
        ...
        while True:
            ...
            if random.choice([1]):
                ...
                hm = self._generate_hm(128, 128, dress_type, new_j, 128, train_weights)
            else:
                new_j = self._relative_joints(cbox, padd, joints, to_size=128)
                hm = self._generate_hm(128, 128, dress_type, new_j, 128, train_weights)
